...
export default function CheckoutPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!json.success) {
        setError(json.error || "فشل إنشاء الطلب");
        return;
      }
      clearCart();
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-border bg-surface p-6">
            <p className="mb-1 text-[11px] font-bold uppercase tracking-[0.18em] text-purple-accent">
              الخطوة 1
            </p>
            ...
            <div className="mt-4 flex flex-col gap-4 sm:flex-row">
              {paypalQr ? (
                ...
                <div className="flex h-44 w-44 shrink-0 flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-deep-purple/40 text-gray-muted">
              ...
              <div className="flex-1">
                ...
                <button
                  type="button"
                  onClick={copyEmail}
                  className="mt-2 flex w-full items-center justify-between gap-2 rounded-xl border border-border bg-deep-purple/40 px-4 py-3 text-sm font-bold text-white transition-colors hover:border-purple-accent/40"
                  dir="ltr"
                >
          ...
          <section className="rounded-2xl border border-border bg-surface p-6">
            ...
            <div className="mt-4">
              ...
              <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-deep-purple/40 px-4 py-6 transition-colors hover:border-purple-accent/50">
                {proofPreview ? (
                  <img
                    src={proofPreview}
                    alt="إثبات الدفع"
                  ...
                  <>
                    ...
                    <span className="text-[13px] text-gray-text">
                      اضغط لاختيار صورة الإيصال (JPG/PNG/WEBP حتى 5MB)
                    </span>
                  </>
                )}
                <input
          ...
          {error && (
            <div className="rounded-xl border border-danger/30 bg-danger/10 p-4 text-[13px] font-semibold text-danger">
              {error}
            </div>
          )}
